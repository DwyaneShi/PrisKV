...
static bool g_zero_copy = false; /* 使用 token 的 ZeroCopy 模式 */
...
/* ===== ZeroCopy token 路径：上下文与回调（需在使用点之前定义） ===== */
typedef struct {
    priskv_context *pctx;
    void (*cb)(int, void *);
    void *cbarg;
...
/* 删除未使用的 ZeroCopy DROP 回调声明 */
/* transfer 路径 GET 回调的前置声明 */
...
static void zc_acquire_cb(uint64_t rid, priskv_status status, void *result)
{
    ...
    if (status != PRISKV_STATUS_OK) {
        priskv_log_warn("ZeroCopy ACQUIRE 失败: status(%d) %s", status, priskv_status_str(status));
        zctx->cb(status, zctx->cbarg);
        free(zctx);
        return;
    }
    ...
    if (region->length > zctx->value_len) {
        priskv_log_warn("ZeroCopy ACQUIRE 长度过大: region.len=%u, user.buf=%u", region->length, zctx->value_len);
...
static void zc_alloc_cb(uint64_t rid, priskv_status status, void *result)
{
    ...
    if (status != PRISKV_STATUS_OK) {
        priskv_log_warn("ZeroCopy ALLOC 失败: status(%d) %s", status, priskv_status_str(status));
        zctx->cb(status, zctx->cbarg);
        free(zctx);
        return;
    }
    ...
    if (copy_len < zctx->value_len) {
        priskv_log_warn("ZeroCopy 写入截断: region.len=%u, request.len=%u, copy_len=%u",
...
/* ZeroCopy DROP 回调已不再使用（已发布键采用 DELETE 语义） */
...
{
    priskv_context *priskv_ctx = ctx;
    /* 删除采用普通 DELETE；DROP 仅用于未发布的 ALLOC token */
...
static void jobs_print_summary(job_context *jobs, int njob)
{
    ...
    if (succ < njob) {
        // 统计并打印异常状态（每个失败的 job 一条）
        printf("Errors:\n");
        for (i = 0; i < njob; i++) {
            if (job_is_error(&jobs[i])) {
                // err_msg 在 job_set_error 中已格式化为 "resp status[%d]: %s"
                printf("  job[%d]: %s\n", i, jobs[i].err_msg);
            }
        }
    }
