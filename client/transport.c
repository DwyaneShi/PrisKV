...
static void priskv_transport_client_keys_reset(priskv_transport_client_impl *impl) {
    if (impl->keys_mem) {
...
static void priskv_transport_send_delayed_req(priskv_client *client)
{
    if (client->impl->ndelayed == 0) return;
    struct delayed_req dq = client->impl->delayed[0];
    memmove(client->impl->delayed, client->impl->delayed + 1, (client->impl->ndelayed - 1) * sizeof(*client->impl->delayed));
    client->impl->ndelayed--;
    priskv_transport_send_am_req(client, dq.buf, dq.len);
}
...
static int submit_req(priskv_client *client, priskv_req_command cmd, const char *key,
                      ...
                      uint64_t request_id, priskv_generic_cb cb)
{
    ...
    if (cmd == PRISKV_COMMAND_KEYS) {
        if (impl->keys_running_id && impl->keys_running_id != request_id) {
            priskv_log_debug("submit_req: keys command is running, request_id 0x%lx\n", impl->keys_running_id);
            if (cb) cb(request_id, PRISKV_STATUS_BUSY, NULL);
            return -1;
        }
        if (!impl->keys_running_id) impl->keys_running_id = request_id;
    }
    ...
    if (nsgl > 0) {
        ...
        if (!auto_mems) {
            priskv_log_error("submit_req: malloc auto_mems failed\n");
            return -ENOMEM;
        }
        for (uint16_t i = 0; i < nsgl; i++) {
            if (!sgl[i].mem) {
                ...
                if (!m) {
                    for (uint16_t j = 0; j < i; j++) {
                        priskv_dereg_memory(auto_mems[j]);
                    }
                    priskv_log_error("submit_req: reg memory failed\n");
                    return -ENOMEM;
                }
                auto_mems[i] = m;
                sgl[i].mem = m;
    ...
    pending_req *req = pending_req_create(impl, request_id, cmd, cb, sgl, nsgl, key, timeout, auto_mems);
    if (!req) {
        priskv_log_error("submit_req: create pending req failed\n");
        return -ENOMEM;
    }
    ...
    if (impl->npending >= impl->max_inflight_command) {
        impl->delayed = realloc(impl->delayed, (impl->ndelayed + 1) * sizeof(*impl->delayed));
        impl->delayed[impl->ndelayed].buf = buf;
        impl->delayed[impl->ndelayed].len = len;
        impl->ndelayed++;
...
static ucs_status_t priskv_transport_am_info_cb(void *arg, const void *header, size_t header_length, void *data, size_t length, const ucp_am_recv_param_t *param)
{
    ...
    priskv_transport_send_delayed_req(impl->owner);
    return UCS_OK;
}
...
static ucs_status_t priskv_transport_am_resp_cb(void *arg, const void *header, size_t header_length, void *data, size_t length, const ucp_am_recv_param_t *param)
{
    ...
    if (p->cb) {
        if (p->cmd == PRISKV_COMMAND_KEYS) {
            if (status == PRISKV_RESP_STATUS_VALUE_TOO_BIG) {
                priskv_transport_client_keys_reset(impl);
                size_t newlen = len + len / 8;
                impl->keys_buf = malloc(newlen);
                if (!impl->keys_buf) {
                    priskv_log_error("priskv_transport_am_resp_cb: recv am resp keys, malloc failed\n");
                    return UCS_OK;
                }
                ...
                if (!impl->keys_mem) {
                    priskv_log_error("priskv_transport_am_resp_cb: recv am resp keys, reg memory failed\n");
                    priskv_transport_client_keys_reset(impl);
                    return UCS_OK;
                }
                priskv_sgl sgl;
                ...
                if (!buf) {
                    priskv_log_error("priskv_transport_am_resp_cb: recv am resp keys, build req buf failed\n");
                    priskv_transport_client_keys_reset(impl);
                    return UCS_OK;
                }
                ...
                if (!keyset) {
                    priskv_log_error("priskv_transport_am_resp_cb: recv am resp keys, malloc keyset failed\n");
                    return UCS_OK;
                }
                ...
                for (uint32_t i = 0; i < nkey; i++) {
                    ...
                    if (!keyset->keys[i].key) {
                        priskv_log_error("priskv_transport_am_resp_cb: recv am resp keys, malloc key failed\n");
                        for (uint32_t j = 0; j < i; j++) {
                            free(keyset->keys[j].key);
                        }
                        free(keyset->keys);
                        free(keyset);
                        return UCS_OK;
    ...
    if (p->cmd == PRISKV_COMMAND_KEYS && status != PRISKV_RESP_STATUS_VALUE_TOO_BIG) {
        impl->keys_running_id = 0;
    }
