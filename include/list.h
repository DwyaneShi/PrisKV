/* taken from rdma-core: merge several changes into a single header file */
 ...
 *
 * See also:
 *	LIST_HEAD_DEFINE, list_head_init()
 *
 * Example:
...
/**
 * LIST_HEAD_DEFINE - define and initialize an empty list_head
 * @name: the name of the list.
 *
 * The LIST_HEAD_DEFINE macro defines a list_head and initializes it to an empty
 ...
 *
 * Example:
 *	static LIST_HEAD_DEFINE(my_global_list);
 */
...
static inline void list_add_after_(struct list_head *h, struct list_node *p, struct list_node *n,
                                   const char *abortstr)
{
    n->next = p->next;
    n->prev = p;
...
static inline void list_add_before_(struct list_head *h, struct list_node *p, struct list_node *n,
                                    const char *abortstr)
{
    n->next = p;
    n->prev = p->prev;
...
#define list_next(h, i, member)                                                                    \
    ((list_typeof(i))list_entry_or_null(list_debug(h, __FILE__ ":" stringify(__LINE__)),           \
                                        (i)->member.next, list_off_var_((i), member)))
...
#define list_prev(h, i, member)                                                                    \
    ((list_typeof(i))list_entry_or_null(list_debug(h, __FILE__ ":" stringify(__LINE__)),           \
                                        (i)->member.prev, list_off_var_((i), member)))
...
static inline void list_append_list_(struct list_head *to, struct list_head *from,
                                     const char *abortstr)
{
...
static inline void list_prepend_list_(struct list_head *to, struct list_head *from,
                                      const char *abortstr)
{
    struct list_node *from_tail = list_debug(from, abortstr)->n.prev;
...
#define list_off_(type, member)                                                                    \
    (container_off(type, member) + check_type(((type *)0)->member, struct list_node))

#define list_off_var_(var, member)                                                                 \
    (container_off_var(var, member) + check_type(var->member, struct list_node))
