...
class PriskvClientTesting:
    ...
    def test_transport_permissions(self):
        ...
        print("[INFO] [TPERM] 测试开始: Transport 权限语义")
        ...
        print(f"[DEBUG] [TPERM] alloc 成功: addr={hex(alloc_region.addr)}, len={alloc_region.length}")
        ...
        try:
            ...
            print(f"[DEBUG] [TPERM] 写入测试数据完成, 写入长度={len(payload)}")
        except Exception as e:
        ...
        print(f"[INFO] [TPERM] release(alloc token) 被拒绝: status={rel_status}")

        # Step3: seal (publish to make value visible)
        seal_status = self.client.seal(TEST_KEY, alloc_region)
        assert seal_status == 0, f"seal failed, status code: {seal_status}"
        print(f"[INFO] [TPERM] seal 成功: status={seal_status}")

        # Step4: acquire (get read-only token)
        acq_status, acq_region = self.client.acquire(TEST_KEY, TIMEOUT)
        assert acq_status == 0 and acq_region.addr != 0
        print(f"[DEBUG] [TPERM] acquire 成功: addr={hex(acq_region.addr)}, len={acq_region.length}")
        ...
        print(f"[INFO] [TPERM] seal(acquire token) 被拒绝: status={seal_status2}")
        ...
        print(f"[INFO] [TPERM] release(acquire token) 成功: status={rel_status2}")

        # Cleanup
        assert self.client.delete(TEST_KEY) == 0
        print("[INFO] [TPERM] 清理完成并删除键")
        print("[INFO] [TPERM] 测试结束: Transport 权限语义")
    ...
    def test_transport_drop_behavior(self):
        ...
        print("[INFO] [TDROP] 测试开始: Transport DROP 语义")
        ...
        print(f"[DEBUG] [TDROP] alloc 未发布: addr={hex(alloc_region.addr)}, len={alloc_region.length}")

        # When unpublished, acquire should be invisible (NO_SUCH_KEY)
        acq_status, _ = self.client.acquire(TEST_KEY, TIMEOUT)
        assert acq_status == priskv.PRISKV_STATUS.PRISKV_STATUS_NO_SUCH_KEY, \
            f"acquire unpublished key expected NO_SUCH_KEY, got {acq_status}"
        print(f"[INFO] [TDROP] acquire 未发布键被拒绝: status={acq_status}")
        ...
        print(f"[INFO] [TDROP] drop(alloc token) 成功: status={drop_status}")
        ...
        print(f"[INFO] [TDROP] drop 后 acquire 仍被拒绝: status={acq_status2}")
        ...
        print(f"[DEBUG] [TDROP] alloc+seal 已发布: addr={hex(alloc_region2.addr)}, len={alloc_region2.length}")

        acq_status3, acq_region = self.client.acquire(TEST_KEY, TIMEOUT)
        assert acq_status3 == 0 and acq_region.addr != 0
        print(f"[DEBUG] [TDROP] acquire 成功: addr={hex(acq_region.addr)}, len={acq_region.length}")
        ...
        print(f"[INFO] [TDROP] drop(acquire token) 被拒绝: status={drop_status2}")

        # Cleanup
        assert self.client.release(TEST_KEY, acq_region) == 0
        assert self.client.delete(TEST_KEY) == 0
        print("[INFO] [TDROP] 清理完成并删除键")
        print("[INFO] [TDROP] 测试结束: Transport DROP 语义")
