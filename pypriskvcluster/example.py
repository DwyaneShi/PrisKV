...
class PriskvClusterExample():
    ...
    def __init__(self, args):
        ...
        ######## PriskvClusterClient ########
        raw_client = priskvcluster.PriskvClusterClient(self.raddr, self.rport, self.password)
        assert (raw_client != 0)
    ...
    def run_example(self):
        ...
        self.reg = self.raw_client.reg_memory(self.get_ptr(),
                                              self.reg_buf_bytes)
        self.sgl = priskvcluster.SGL(self.get_ptr(), self.reg_buf_bytes, self.reg)
...
class PriskvClusterNumpyExample(PriskvClusterExample):
    ...
    def run_example(self):
        ...
        new_reg = self.raw_client.reg_memory(new_reg_buf.ctypes.data,
                                             new_reg_buf_bytes)
        new_sgl = priskvcluster.SGL(new_reg_buf.ctypes.data, new_reg_buf_bytes, new_reg)
...
def example(args):
    priskv_example = PriskvClusterNumpyExample(args)
