...
class PriskvClientTesting:
    ...
    def __init__(self, raddr: str, rport: int, password: str):
        self.client = priskvcluster.PriskvClusterClient(raddr, rport, password)
        self.key = "priskvcluster-testing-key"
    ...
    def set(self):
        assert self.client.set(
            self.key,
            priskvcluster.SGL(self.sendbuf.ctypes.data, self.sendbuf.nbytes,
                     self.sendmr), 1) == 0

    def get(self):
        assert self.client.get(
            self.key,
            priskvcluster.SGL(self.recvbuf.ctypes.data, self.recvbuf.nbytes,
                     self.recvmr), 1) == 0
    ...
    def test_mset(self):
        ...
        sgls = [
            priskvcluster.SGL(self.sendbuf.ctypes.data + i * byte_len, byte_len,
    ...
    def test_mget(self):
        ...
        recv_sgls = [
            priskvcluster.SGL(self.recvbuf.ctypes.data + i * byte_len, byte_len,
...
def run_testing(testing):
    ...
    print("test priskvcluster success!")


def main():
    parser = argparse.ArgumentParser(description='Priskv Cluster Example')
