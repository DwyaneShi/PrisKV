...
void priskv_info_get_connection(void *data)
{
    ...
    priskv_ucp_listener *listeners = priskv_ucp_get_listeners(&info->nlisteners);
    info->listeners = calloc(info->nlisteners, sizeof(priskv_conn_listener_info));

    for (int i = 0; i < info->nlisteners; i++) {
        priskv_ucp_listener *listener = &listeners[i];
        ...
        for (int j = 0; j < listener->nclients; j++) {
            priskv_ucp_client *client = &listener->clients[j];
    ...
    priskv_ucp_free_listeners(listeners, info->nlisteners);
}
