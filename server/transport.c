...
static void priskv_check_and_log_slow_query(priskv_transport_rw_work *work)
{
...
static void priskv_transport_rw_complete_cb(void *request, ucs_status_t status, void *user_data)
{
    ...
    priskv_transport_send_response(w->ep, be64toh(w->req->request_id), PRISKV_RESP_STATUS_OK, w->len);
    priskv_transport_destroy_segs(w->segs, w->nsgl);
    priskv_check_and_log_slow_query(w);
    free(w);
}

typedef struct priskv_transport_keys_cleanup_ctx {
    ucp_mem_h memh;
    uint8_t *buf;
} priskv_transport_keys_cleanup_ctx;

static void priskv_transport_keys_cleanup(void *arg)
{
    priskv_transport_keys_cleanup_ctx *c = (priskv_transport_keys_cleanup_ctx *)arg;
    if (!c) return;
    if (c->memh) ucp_mem_unmap(g_server.context, c->memh);
    if (c->buf) free(c->buf);
    free(c);
}
                                ...
                                int is_set)
{
    ...
    for (uint16_t i = 0; i < nsgl; i++) {
        ...
        void *r = NULL;
        if (is_set) {
            r = ucp_get_nbx(ep, buf + off, slen, segs[i].addr, segs[i].rkey, &p);
        } else {
            r = ucp_put_nbx(ep, buf + off, slen, segs[i].addr, segs[i].rkey, &p);
        }
        if (UCS_PTR_IS_ERR(r)) {
...
int priskv_transport_send_response(ucp_ep_h ep, uint64_t request_id, priskv_resp_status status, uint32_t length)
{
    ...
    priskv_transport_am_send(ep, priskv_transport_am_id_resp, resp, sizeof(*resp), priskv_transport_send_done_cb, resp);
    return 0;
}
...
static ucs_status_t priskv_transport_am_req_cb(void *arg, const void *header, size_t header_length, void *data, size_t length, const ucp_am_recv_param_t *param)
{
    ...
    if (priskv_get_log_level() >= priskv_log_debug) {
        char key_short[16] = {0};
        priskv_string_shorten(keyptr, keylen, key_short, sizeof(key_short));
        uint16_t key_short_len = (uint16_t)strlen(key_short);
        priskv_log_debug("priskv_transport_am_req_cb: recv am req, id %lu, command %s[0x%x], key %.*s, nsgl %u, keylen %u\n", request_id, priskv_command_str(command), command, key_short_len, key_short, nsgl, keylen);
    }
    ...
    case PRISKV_COMMAND_KEYS: {
        ...
        priskv_transport_keys_cleanup_ctx *ctx = malloc(sizeof(*ctx));
        if (!ctx) {
        ...
        ctx->memh = memh;
        ctx->buf = keysbuf;
        if (priskv_transport_rw_submit(reply_ep, req, segs, nsgl, keysbuf, reallen, priskv_transport_keys_cleanup, ctx, 0) != 0) {
            priskv_transport_keys_cleanup(ctx);
            resp->status = htobe16(PRISKV_RESP_STATUS_SERVER_ERROR);
            resp->length = htobe32(0);
            break;
        }
